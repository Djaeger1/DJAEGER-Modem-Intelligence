...
# UCI file declares: config core 'main'. Address it by named section main, not package.type.section.
uci -q get djaeger_modem.main >/dev/null 2>&1||uci set djaeger_modem.main=core
for KV in self_heal=1 allow_wan_reconnect=1 allow_service_recovery=1 allow_dnsmasq_restart=1 allow_netifd_restart=1 allow_modem_post=1 allow_modem_reboot=1 autonomous_root_recovery=1 traffic_governor=0 broker_wan_cooldown=30 broker_wan_max_hour=6 broker_dns_cooldown=120 broker_dns_max_hour=3 broker_netifd_cooldown=120 broker_netifd_max_hour=3 modem_reboot_cooldown=120 max_modem_reboots_hour=3;do K=${KV%%=*};V=${KV#*=};uci set "djaeger_modem.main.$K=$V"||rollback;done;uci commit djaeger_modem||rollback
