...
BASE='https://raw.githubusercontent.com/Djaeger1/DJAEGER-Modem-Intelligence/main';TMP="/tmp/djaeger-release.$$";TS="$(date +%s)";RB="/root/djaeger-modem/rollback/release-$TS";mkdir -p "$TMP" "$RB"
...
# Snapshot every path this release can replace.
...
chmod 755 /usr/sbin/djaeger-* /etc/init.d/djaeger-modem;chmod 644 /usr/lib/djaeger-modem/*.sh /usr/share/djaeger-modem/providers/xl.policy
# Production authorities. Governor remains OFF until sufficient runtime evidence and explicit opt-in.
...
/etc/init.d/djaeger-modem enable >/dev/null 2>&1||true;/etc/init.d/djaeger-modem start||rollback;sleep 5
ps w|grep -q '[d]jaeger-modemd'||rollback
# Read-only cellular engine smoke test. Never trigger an intentional outage/reboot during installation.
/usr/lib/djaeger-modem/cellular-state-engine.sh >/dev/null 2>&1||rollback
[ -s /tmp/djaeger-modem/cellular-state ]||rollback
rm -rf "$TMP";trap - INT TERM HUP
