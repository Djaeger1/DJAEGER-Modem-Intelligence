...
echo 'DJAEGER Modem Intelligence v1.2 Adaptive + Gemini Candidate (r3)'
...
i=0
while [ $i -lt 20 ] && ps w | grep -q '[d]jaeger-modemd'; do sleep 1; i=$((i+1)); done
if ps w | grep -q '[d]jaeger-modemd'; then echo 'Old daemon did not stop safely; candidate not installed.'; exit 1; fi
# No daemon is alive now. Remove only obsolete singleton residue from the previous generation.
...
if [ ! -f /etc/config/djaeger_modem ]; then
 ...
 for KV in "adaptive_intelligence=1" "baseline_min_samples=40" "gemini_enabled=0" "gemini_model=gemini-2.5-flash" "gemini_timeout=8" "gemini_min_interval=900" "gemini_risk_threshold=50"; do
  K=${KV%%=*}; V=${KV#*=}; uci -q get "djaeger_modem.main.$K" >/dev/null || uci set "djaeger_modem.main.$K=$V"
 done
...
# Runtime acceptance requires a live process plus a state generated by this exact start.
...
if [ "$OK" != 1 ]; then
 ...
 echo "Rollback restored from $RB"
 exit 1
fi
rm -rf "$TMP"
echo 'Candidate r3 installed and runtime-verified.'
echo 'Adaptive Local Intelligence enabled; Gemini remains disabled.'
